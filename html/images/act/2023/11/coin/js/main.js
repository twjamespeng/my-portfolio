...
const prizeValue = document.querySelector('#prizeValue') // 金幣
...
async function getAvyAction () {
  try {
    const { data: { code, data } } = await instance.post(`${BASE_URL}/api/AvyAction/SinglesDay`, {
      memberId,
      token
    })
...
async function checkedAvyActionInspect () {
  try {
    ...
    const { data: { code, data } } = await instance.post(`${BASE_URL}/api/AvyAction/SinglesDay/Inspect`, {
      memberId,
      token
    })
