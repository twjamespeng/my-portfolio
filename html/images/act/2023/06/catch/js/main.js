...
const alreadyFull = new bootstrap.Modal(document.querySelector('#alreadyFull'))
const alreadyPlayed = new bootstrap.Modal(document.querySelector('#alreadyPlayed'))
...
async function checkedAvyActionInspect () {
  try {
    ...
    // const { data: { code, data } } = await instance.post(`${BASE_URL}/api/AvyAction/Zongzi/Inspect`, {
    //   memberId,
    //   token
    // })
...
async function loginDemo () {
  ...
  try {
    ...
    const res = { data: { code: 200, data: { memberId: '123456789', token: '123456789' } } }
    if (res.data.code === 200) {
      setLoginCookies(res.data.data)
      loginModal.hide()

      // const isQualifications = await checkedAvyActionInspect()
      const isQualifications = true // 先不檢查資格
