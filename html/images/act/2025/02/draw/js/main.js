...
async function draw () {
  try {
    // const { data: { code, data } } = await instance.post(`${BASE_URL}/api/AvyAction/SpringOuting`, {
    //   memberId,
    //   token
    // })

    code = 200;
    data = { avy_code: '80050' } // 測試用

    if (code === 200) {
      const { avy_code: avyCode } = data

      const avyCodeMap = {
        80050: { name: '1', title: '恭喜獲得：紅利點數', value: '50點' },
        60001: { name: '3', title: '恭喜獲得：', value: '麥當勞蛋捲冰淇淋' },
        10100: { name: '6', title: '恭喜獲得：銀幣', value: '$100元' },
...
async function checkedAvyActionInspect () {
  try {
    ...
    const { data: { code, data } } = await instance.post(`${BASE_URL}/api/AvyAction/SpringOuting/Inspect`, {
      memberId,
      token
    })
